...
static struct urltestcase urllist[] = {
  {"file:///file.txt#moo",
   "file:///file.txt#moo",
   0, 0, CURLURLE_OK},
  {"file:////file.txt",
...
static struct redircase rellist[] = {
...
static int set_url(void)
{
  ...
  for(i = 0; rellist[i].in; i++) {
    char *url = NULL;
    urlp = NULL;
    rc = curl_url(rellist[i].in, &urlp, rellist[i].urlflags);
    if(!rc) {
      rc = curl_url_set(urlp, CURLUPART_URL, rellist[i].set,
                        rellist[i].setflags);
      if(rc) {
        fprintf(stderr, "%s:%d Set URL %s returned %d\n",
                __FILE__, __LINE__, rellist[i].set,
                (int)rc);
        return 5;
      }
      ...
      fprintf(stderr, "Set URL\nin: %s\nreturned %d (expected %d)\n",
              rellist[i].in, (int)rc, rellist[i].ucode);
      return 4;
    }
...
static int set_parts(void)
{
  ...
  CURLUcode rc;
  CURLURL *urlp;
  for(i = 0; setlist[i].set; i++) {
    char *url = NULL;
    urlp = NULL;
    rc = curl_url(setlist[i].in, &urlp, setlist[i].urlflags);
    if(!rc) {
    ...
    else if(rc != setlist[i].ucode) {
      fprintf(stderr, "Set parts\nin: %s\nreturned %d (expected %d)\n",
              setlist[i].in, (int)rc, setlist[i].ucode);
      return 4;
    }
    curl_free(url);
    curl_url_cleanup(urlp);
...
static int get_url(void)
{
  ...
  for(i = 0; urllist[i].in; i++) {
    char *url = NULL;
    urlp = NULL;
    rc = curl_url(urllist[i].in, &urlp, urllist[i].urlflags);
    if(!rc) {
      ...
      fprintf(stderr, "Get URL\nin: %s\nreturned %d (expected %d)\n",
              urllist[i].in, (int)rc, urllist[i].ucode);
      return 4;
    }
    curl_free(url);
    curl_url_cleanup(urlp);
...
static int get_parts(void)
{
  ...
  CURLURL *urlp;
  for(i = 0; list[i].in; i++) {
    rc = curl_url(list[i].in, &urlp, list[i].urlflags);
    if(rc != list[i].ucode) {
      fprintf(stderr, "Get parts\nin: %s\nreturned %d (expected %d)\n",
              list[i].in, (int)rc, list[i].ucode);
      return 1;
    }
