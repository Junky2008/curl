 ...
 */
static CURLUcode parseurl(const char *url, struct Curl_URL *u,
                          unsigned int flags)
{
  char *path;
...
/*
 */
CURLUcode curl_url(const char *url, CURLURL **urlhandle, unsigned int flags)
{
  struct Curl_URL *u;
  CURLUcode result = CURLURLE_OK;
  *urlhandle = NULL;
  ...
  if(!url && (flags & CURLURL_VERIFY_ONLY))
    /* a NULL URL does not verify */
    result = CURLURLE_MALFORMED_INPUT;
  else
